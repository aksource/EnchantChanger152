...
@Mod(modid="VisibleRayGenerator", name="VisibleRayGenerator", version="build 3 (for mc1.5.2  ic21.115.349if  Forge#737)", dependencies ="required-after:IC2")
...
//		isICstacksizeUp = config.get(Configuration.CATEGORY_GENERAL, "isICstacksizeUp", true, "I will up the stacksize of IC2 some items").getBoolean(true);
	...
	public void load(FMLInitializationEvent event)
	{
		...
		ModLoader.registerBlock(solarBlock, CESolarItem.class);
		lavaUpdater = new CEItemLavaUpdater(itemID_lavaUpdater).setUnlocalizedName(TexDomain + "CE_LavaUpdater");
...
*/
		ModLoader.addRecipe(new ItemStack(this.lavaUpdater), "c", "a", "i", 
		...
		ModLoader.addRecipe(rlg[5], "psp","gmg","psp", 
			'p',Items.getItem("coalChunk"), 'g',solar4, 'm',Items.getItem("mvTransformer"), 's',Block.glowStone);
			
		for(int i = 0; i < 10; i++)
		{
			ModLoader.addRecipe(rlg[i+1], "pgp","gmg","pgp", 'p',plates[i], 'g',rlg[i], 'm',items[i]);
		}
